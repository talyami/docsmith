...
import { UploadCloud, FileType, CheckCircle2, AlertCircle, Loader2, Download, ArrowRight, FileCheck, RefreshCw } from "lucide-react";
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        <div className="lg:col-span-2 space-y-6">
          ...
          >
            ...
            >
              <div className={`flex flex-col items-center justify-center text-center space-y-5 transition-all duration-500 ${isCollapsed ? "p-4" : "p-14 min-h-[360px]"}`}>
                <div className={`bg-primary/10 rounded-full text-primary transition-all duration-500 ${isCollapsed ? "p-2" : "p-5"}`}>
                  <UploadCloud className={`transition-all duration-500 ${isCollapsed ? "h-5 w-5" : "h-12 w-12"}`} />
                </div>
                {!isCollapsed && (
                  <>
                    <div className="space-y-1">
                      <h3 className="text-2xl font-semibold">Drag & Drop your file here</h3>
                      <p className="text-muted-foreground text-sm">Or click anywhere to browse your computer</p>
                    </div>
                    <input
                      type="file"
                      ref={fileInputRef}
                    ...
                    <Button
                      ...
                      className="px-10 shadow hover:shadow-md transition-all"
                    >
                      {isUploading ? (
          ...
          {/* Result Card — slides in and expands after upload */}
          {activeConversion && (
            <div
              style={{
                ...
                <div className="p-6 space-y-5">
                  <div className="flex items-start justify-between">
                    <div className="flex items-center gap-3">
                      <div className={`p-2.5 rounded-xl ${activeConversion.status === "done" ? "bg-green-500/10 text-green-600" : "bg-primary/10 text-primary"}`}>
                        {activeConversion.status === "done" ? <FileCheck className="h-6 w-6" /> : <FileType className="h-6 w-6" />}
                      </div>
                      <div>
                        <h4 className="font-semibold text-foreground">{activeConversion.originalFilename}</h4>
                  ...
                  {activeConversion.status === "done" && activeConversion.downloadPath && (
                    <a
                      href={activeConversion.downloadPath}
                      download
            ...
            <div className="space-y-4 pt-2">
              ...
              <div className="grid gap-3">
                {recentConversions.map((conv) => (
                  <Card key={conv.id} className="p-4 flex items-center justify-between hover:border-primary/40 transition-colors">
                    <div className="flex items-center gap-3">
...
function StatusBadge({ status }: { status: Conversion["status"] }) {
  if (status === "done") {
    return (
      <span className="flex items-center text-xs font-medium text-green-600 bg-green-500/10 px-2.5 py-1 rounded-full border border-green-500/20">
  ...
  if (status === "processing") {
    return (
      <span className="flex items-center text-xs font-medium text-blue-600 bg-blue-500/10 px-2.5 py-1 rounded-full border border-blue-500/20">
